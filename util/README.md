...
| [`generate_dataset_summary_table.py`](./generate_dataset_summary_table.py) | Generate pivot tables of unique subject/sample counts and subject diagnosis counts by organism × tissue type × assay from CRN Cloud summary outputs. | Run after `crn_cloud_collection_summary` to produce summary tables for reporting. Reads from the Google Releases Sheet via `get_releases_df()`. | `python3 generate_dataset_summary_table.py crn_cloud_collection_summary.<date>.tsv subject_dataset_membership.<date>.tsv sample_dataset_membership.<date>.tsv subject_diagnosis_membership.<date>.tsv` |
...
- Subject and sample membership files contain one row per ID-dataset pair; global deduplication is performed by `generate_dataset_summary_table.py`
